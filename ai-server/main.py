...
try:
    from sensor_engine import evaluate_sensor_rules
except ImportError:
    # Safe fallback if sensor_engine.py is missing or failing
    def evaluate_sensor_rules():
        return {
            "status": "sensor_engine_missing",
            "stress_level": "NORMAL",
...
# Data Model for Direct Sensor Group HTTP Streams
...
# Enable CORS for cross-device network calls (Unity VR headset / external sensors)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def translate_audio(request: Request):
    try:
        ...
        # 4. Transcribe audio array directly
        ...
        # 7. Evaluate Sensor Rule Matrix for live GUI trigger and AI Action
        sensor_eval = evaluate_sensor_rules()

        # 8. Return comprehensive JSON payload required by the VR team
        return {
            ...
            "gui_trigger": sensor_eval.get("gui_trigger", "DEFAULT_UI"),
            "ai_action": sensor_eval.get("ai_action", "STANDARD_DIALOGUE"),
            "sensor_state": sensor_eval
        }
...
async def receive_sensor_data(data: SensorData):
    """
    Endpoint for the Sensors Group to post live biometric/EDA data via HTTP.
    """
    sensor_logs.append(data.dict())
    print(f"📊 Received Live Sensor Data: {data.dict()}")
...
async def get_latest_sensor_data():
    """
    Returns the most recent HTTP sensor reading.
    """
    if not sensor_logs:
        return {"status": "no data received yet"}
    return sensor_logs[-1]


@app.get("/sensor-engine/status")
async def get_sensor_engine_status():
    """
    Returns the current evaluation of ai_input_feature.csv against the Sensor-AI Rule Matrix.
    """
    return evaluate_sensor_rules()
import string
from contextlib import asynccontextmanager
from fastapi import FastAPI, Request, HTTPException
import numpy as np
